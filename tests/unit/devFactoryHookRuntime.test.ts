...
describe("devFactoryHookRuntime", () => {
  ...
  it("HK-11 stop hook emits /summarize in factory sessions when latch armed", () => {
    const root = fakeEngine();
    const res = decideDevFactoryStopHook({
      engineRoot: root,
      status: "completed",
  ...
  it("HK-12 isFactoryHookSession detects background and env", () => {
    ...
    expect(
      isFactoryHookSession({ env: { CURSOR_FACTORY_SESSION: "1" } }),
    ).toBe(true);
    expect(isFactoryHookSession({})).toBe(false);
  });
});
